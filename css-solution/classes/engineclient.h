...
public:
	...
	// If bClamp is specified, the resulting vec3_t is restricted to the 0.0 to 1.0 for each element
	...
	virtual IMaterial			*TraceLineMaterialAndLighting(const vec3_t &start, const vec3_t &end,
	...
	virtual bool				GetPlayerInfo(int ent_num, tPlayerInfo *pinfo) = 0;
	...
	virtual void				Con_NPrintf(int pos, _Printf_format_string_ const char *fmt, ...) = 0;
	// Similar to Con_NPrintf, but allows specifying custom text color and duration information
	virtual void				Con_NXPrintf(const struct con_nprint_s *info, _Printf_format_string_ const char *fmt, ...) = 0;
	...
	virtual const matrix_t& 		WorldToScreenMatrix() = 0;
	...
	virtual const matrix_t& 		WorldToViewMatrix() = 0;
	...
	virtual bool		IsOccluded(const vec3_t &vecAbsMins, const vec3_t &vecAbsMaxs) = 0;
	...
	virtual void		DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial, matrix_t& transform, const color &color) = 0;
	...
	virtual SkyboxVisibility_t IsSkyboxVisibleFromPoint(const vec3_t &vecPoint) = 0;
...
#if defined( USE_SDL )
	// we need to pull delta's from the cocoa mgr, the engine vec3_ts this for us
